        ...
        <div className={styles.timelineContainer}>
          ...
          <div className={styles.timelineGrid}>
            {timelineSteps.map((step, index) => {
              ...
              const scatterX = dir * (50 + Math.random() * 50); // reduced from 200 to keep it visible
              const scatterY = (Math.random() - 0.5) * 100; // reduced from 300
              const scatterRotate = (Math.random() - 0.5) * 45;

              // Waypoints to simulate fast buzzing flies before settling
              const p1X = scatterX + dir * 20;
              const p1Y = scatterY + (Math.random() - 0.5) * 50;
              const r1 = scatterRotate + (Math.random() - 0.5) * 60;

              const p2X = scatterX * 0.3 + (Math.random() - 0.5) * 50;
              const p2Y = scatterY * 0.3 + (Math.random() - 0.5) * 50;
              const r2 = scatterRotate * 0.5 + (Math.random() - 0.5) * 30;

              const p3X = (Math.random() - 0.5) * 20;
              const p3Y = (Math.random() - 0.5) * 20;
